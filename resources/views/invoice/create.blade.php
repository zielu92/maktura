            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <div class="flex flex-wrap -mx-3 mb-6"  x-data="{ buyer_id: '-1' }">

                            <div class="w-full md:w-1/4 px-3 py-2">
                                <x-input-label for="status" :value="__('Status')" />
                                <select name="status" id="type" class="mt-1 block w-full dark:bg-gray-900  sm:rounded-lg">
                            ...
                            <div class="w-full md:w-1/4 px-3 py-2">
                                ...
                                <select name="type" id="type" class="mt-1 block w-full dark:bg-gray-900  sm:rounded-lg">
                            ...
                            <div class="w-full md:w-1/4 px-3 py-2">
                                ...
                                <x-text-input id="no" name="no" type="text"
                                    class="mt-1 block w-full" :value="old('no')" required autofocus
                                    autocomplete="no" value="/{{date('Y')}}" />
                            ...
                            <div class="w-full md:w-1/4 px-3 py-2">
                                ...
                                <select name="payment_status" id="payment_status" class="mt-1 block w-full dark:bg-gray-900  sm:rounded-lg">
                            ...
                            <div class="w-full md:w-1/6 px-3 py-2">
                                ...
                                <select name="payment_method_id" id="payment_method_id" class="mt-1 block w-full dark:bg-gray-900  sm:rounded-lg">
                                    <option value="cash" selected='selected'>Gotówka</option>
                            ...
                            <div class="w-full md:w-1/6 px-3 py-2">
                                ...
                                <x-text-input id="due_date" name="issue_date" type="date" class="mt-1 block w-full"
                            ...
                            <div class="w-full md:w-1/4 px-3 py-2">
                                ...
                                <x-text-input id="issuer_name" name="postal_code" type="text"
                            ...
                            <div class="w-full md:w-1/4 px-3 py-2">
                                ...
                                <select x-model="buyer_id" name="buyer_id" id="buyer_id" class="mt-1 block w-full dark:bg-gray-900  sm:rounded-lg">
                                    <option value="-1" selected='selected' >Dodaj nowego</option>
                                    @foreach ($buyers as $buyer)
                                        <option value="{{$buyer->id}}">{{$buyer->name}}({{$buyer->company_name}})</option>
                                    @endforeach
                                </select>
                        ...
                        <div x-data="invoiceItems()" class="w-full px-1 py-2">
                            ...
                            <template x-for="(item, index) in items" :key="item.id">
                              <div class="flex flex-wrap -mx-3 mb-6 dark:bg-gray-700 sm:rounded-lg px-3 py-2">
                                <div class="md:w-1/5">
                                    <x-input-label for="item[][name]" :value="__('Nazwa przedmiotu/usługi')" />
                                    <textarea rows="4" name="item[][name]" class="mt-1 block w-full dark:bg-gray-900 sm:rounded-lg" required autofocus resize-none>{{old('item[][name]')}}</textarea>
                                ...
                                <div class="flex flex-wrap md:w-4/5">
                                    <div class="w-full md:w-1/6 px-6">
                                        ...
                                        <x-text-input id="item[][quantity]" name="item[][quantity]" type="number" min="1" x-model="item.quantity" class="mt-1 block w-full"
                                            :value="old('item[][quantity]')" required autocomplete="item[][quantity]" x-on:change.debounce="calculateItem(item)"/>
                                    ...
                                    <div class="w-full md:w-1/4 px-6">
                                        ...
                                        <x-text-input id="item[][price_net]" name="item[][price_net]" type="number" x-model="item.price_net"  min="0.00" step="0.01" class="mt-1 block w-full"
                                            :value="old('item[][price_net]')" required autocomplete="item[][price_net]" x-on:change.debounce="calculateItem(item)"/>
                                        <x-input-error class="mt-2" :messages="$errors->get('item[][price_net]')" />
                                    </div>
                                    <div class="w-full md:w-1/4 px-6">
                                        <x-input-label for="item[][tax_rate]" :value="__('Vat')" />
                                        <select name="item[][tax_rate]" id="item[][tax_rate]" class="mt-1 block w-full dark:bg-gray-900  sm:rounded-lg" x-model="item.tax_rate" x-on:change.debounce="calculateItem(item)">
                                    ...
                                    <div class="w-full md:w-1/4 px-6">
                                        ...
                                        <x-text-input id="item[][discount]" name="item[][discount" type="text"  value="0" class="mt-1 block w-full"
                                            :value="old('item[][discount]')" required autofocus autocomplete="item[][discount]" x-model="item.discount" x-on:change.debounce="calculateItem(item)"/>
                                    ...
                                    <div class="w-full md:w-1/4 px-6">
                                        ...
                                        <x-text-input type="text" x-model="item.price_gross" class="mt-1 block w-full" disabled />
                                    </div>
                                    <div class="w-full md:w-1/4 px-6">
                                        <x-input-label :value="__('Suma netto')" />
                                        <x-text-input type="text" x-model="item.total_price_net" class="mt-1 block w-full" disabled />
                                    </div>
                                    <div class="w-full md:w-1/4 px-6">
                                        <x-input-label :value="__('Suma brutto')" />
                                        <x-text-input type="text" x-model="item.total_price_gross" class="mt-1 block w-full" disabled />
                                    </div>

                                    <div class="w-full md:w-1/4 px-6">
                                        <x-input-label :value="__('Suma upustów')" />
                                        <x-text-input type="text" x-model="item.total_price_discount" class="mt-1 block w-full" disabled />
                                    </div>
                                </div>
                              </div>
                            </template>

                            <x-danger-button @click.prevent="addNewField()">+ Dodaj pozycję na fakturze</x-danger-button>
                        </div>
        ...
        function invoiceItems() {
                return {
                    ...
                    calculateItem(item) {
                        ...
                        calculatedItem.total_price_net = (calculatedItem.quantity * (calculatedItem.price_net-calculatedItem.discount)).toFixed(2)
                        //TODO: extend and make more dynamic
                        //calculate gross price per unit
                        switch(calculatedItem.tax_rate)
                        {
                            case "23":
                                calculatedItem.price_gross = ((calculatedItem.price_net-calculatedItem.discount)*1.23).toFixed(2)
                                break;
                            case "22":
                                calculatedItem.price_gross = ((calculatedItem.price_net-calculatedItem.discount)*1.22).toFixed(2)
                                break;
                            case "8":
                                calculatedItem.price_gross = ((calculatedItem.price_net-calculatedItem.discount)*1.08).toFixed(2)
                                break;
                            case "5":
                                calculatedItem.price_gross = ((calculatedItem.price_net-calculatedItem.discount)*1.05).toFixed(2)
                                break;
                            default:
                                calculatedItem.price_gross = ((calculatedItem.price_net-calculatedItem.discount)).toFixed(2)
                                break;
                        }
                        //sum gross price
                        calculatedItem.total_price_gross = (calculatedItem.price_gross*calculatedItem.quantity).toFixed(2)
                        //sum discounts
                        calculatedItem.total_price_discount = (calculatedItem.discount*calculatedItem.quantity).toFixed(2)
                    }
