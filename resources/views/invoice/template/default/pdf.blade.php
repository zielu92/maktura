...
<div class="max-w-7xl mx-auto">
    ...
    <section class="float-right text-right px-10 py-5">
        <p class="text-xs">
    ...
    <section class="flex justify-between px-10 mr-5 py-5">
        <p class="text-xs">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tr class="bg-white">
                    ...
                    <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 text-right">
                        {{$item->price_net}} {{$invoice->currency}}
                    </td>
                    <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 text-right">
{{--                        TODO: WRITE HELPER--}}
                    ...
                    @endif
                    <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 text-right">
                        {{$item->tax_amount}} {{$invoice->currency}}
                    </td>
                    @if($showDiscount)
                        <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 text-right">
                            {{$item->total_discount}} {{$invoice->currency}}
                        </td>
                    @endif
                    <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 text-right">
                        {{$item->price_gross}} {{$invoice->currency}}
                    </td>
                </tr>
                @endforeach
                @if($invoice->total_gross!=$invoice->total_net)
                    <tr class="bg-gray-50">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 text-right font-medium">
                            {{$invoice->total_net}} {{$invoice->currency}}
                        </td>
                    </tr>
                @endif
                ...
                @if($invoice->total_tax!='0.00')
                    <tr class="bg-gray-50">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-xs text-gray-500 text-right font-medium">
                            {{$invoice->total_tax}} {{$invoice->currency}}
                        </td>
                    </tr>
                @endif
                <tr class="bg-gray-50">
                    ...
                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 text-right font-medium">
                        {{$invoice->total_gross}} {{$invoice->currency}}
                    </td>
                </tr>
                </tbody>
            </table>
